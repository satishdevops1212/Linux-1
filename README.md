# Linux-1
complete
# linux acces through ip address

ssh -l userame ipaddress
whoami - current user info
hostname - 
htop - gives the all info about 
ex : ext4 - current standrad linux, ext3 - olderlinux , NTFS - New technology file systems - 16TB max file size
, FAT- File allocation Table
/etc - sys config files
/home - user files
/var/log - logfiles
/bin, /usr/bin - commands, scripts, /sbin - file system commands
sudo -su it will tranfers to root user or cd /

_____________________________________________________________

pwd 

cd : 

ls : ls -R, ls -al [ includes with permissions] , Ls -Ltr

cat - cat .profile 

cp - cp file2.txt file3.txt

mv - mv file3.txt file4.txt , ls -l [ check the output]

mkdir - mkdir aspin, cd aspin, pwd

rm
